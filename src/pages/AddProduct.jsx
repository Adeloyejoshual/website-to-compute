...
export default function AddProduct({ session }) {
  ...
  // Upload image to Cloudinary in per-product folder
  const uploadImage = async (file, productId) => {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("upload_preset", UPLOAD_PRESET);
    formData.append("folder", `chatImages/${productId}`); // per-product folder
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: product, error: productError } = await supabase
        .from("products")
        .insert([{ title, price: Number(price), seller_id: userId, stock: 1 }])
        .select()
        .single();
      if (productError) throw productError;

      // 2️⃣ Upload images into per-product folder
      const imageRecords = [];
    ...
    <div style={{ maxWidth: 500, margin: 40, padding: 24, borderRadius: 12, background: "#fff" }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <input
          ...
          style={{ padding: 14, border: "1px solid #d1d5db", borderRadius: 8 }}
          required
        />
        ...
        <input
          ...
          style={{ padding: 14, border: "2px dashed #d1d5db", borderRadius: 8 }}
        />

        {images.length > 0 && (
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", padding: 16, background: "#f8fafc", borderRadius: 8, border: "1px solid #e2e8f0" }}>
            {images.map((img, i) => (
              <img
                key={i}
                src={URL.createObjectURL(img)}
                alt={`Preview ${i+1}`}
                style={{ width: 80, height: 80, objectFit: "cover", borderRadius: 8, border: "2px solid #3b82f6" }}
              />
            ))}
            <span style={{ alignSelf: "center", color: "#6b7280" }}>{images.length} image(s)</span>
          </div>
        )}
        ...
        <button
          ...
          style={{ padding: 16, background: "#10b981", color: "white", border: "none", borderRadius: 8, fontSize: 16, cursor: "pointer" }}
        >
